...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        cursor.executemany(
            ...
            [
                ...
                (2, "咖啡廳",            "2026-05-28", "19:00", "21:00", "咖啡讀書會",   "已確認"),
    ...
    cursor.execute("SELECT id FROM spaces WHERE name = '小角 . 手捻咖啡'")
    _old = cursor.fetchone()
    if _old:
        cursor.execute(
            "UPDATE spaces SET name = ?, address = ?, lat = ?, lng = ? WHERE id = ?",
            ("咖啡廳", "臺中市南屯區", 24.1380, 120.6430, _old[0]),
        )
        cursor.execute(
            "UPDATE bookings SET space_name = ? WHERE space_name = ?",
            ("咖啡廳", "小角 . 手捻咖啡"),
